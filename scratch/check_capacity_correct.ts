...
import { getEffectiveDailyLimit } from '../src/features/email/lib/warmup-engine';
...
async function run() {
  ...
  for (const p of providers) {
    ...
    const available = Math.max(0, effectiveLimit - (p.sentToday || 0));
    console.log(`${p.provider}: Tier=${p.accountTier}, Limit=${effectiveLimit}, SentToday=${p.sentToday}, Available=${available}`);
